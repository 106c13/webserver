...
int main(int argc, char** argv) {
    ...
    // 413 oversized body: 2MB POST against 1MB-capped location
    {
        std::string body(2 * 1024 * 1024, 'X');
        std::string req = buildRequest(
            "POST", "/", t.host,
            "Content-Type: application/octet-stream\r\n", body);
        std::string res = doRequest(t.host, p.php, req);
        printResult("POST 2MB → 413", parseStatus(res) == 413);
    }
    ...
    // Keep-alive: two requests on one socket
    {
        ...
        sendRequest(sock, req1);
        std::string r1 = readOneResponse(sock);

        std::string req2 = "GET / HTTP/1.1\r\nHost: " + t.host
                         + "\r\nConnection: close\r\n\r\n";
        sendRequest(sock, req2);
        std::string r2 = readResponse(sock);
        close(sock);

        bool ok = parseStatus(r1) == 200 && parseStatus(r2) == 200;
